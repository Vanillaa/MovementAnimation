...
public class LazSpriteanimation extends ApplicationAdapter {
	...
	boolean[] arbDirection = new boolean[4];// 2=right, 3=left
	...
	public void create() {
		...
		taKnight = new TextureAtlas(Gdx.files.internal("KnightSpriteSheet.pack"));
		...
		for (int j = 0; j <= 1; j++) {
			atrFront[i] = taKnight.findRegion("frame-" + i);
			i++;

		}
		animLeft = new Animation(0.15f, atrLeft);
		for (int j = 0; j <= 1; j++) {
			atrRight[j] = taKnight.findRegion("frame-" + i);
			i++;
		}
